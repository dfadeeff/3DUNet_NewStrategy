...
class AdaptiveMultiScaleLoss(nn.Module):
    def __init__(self, num_scales=4, alpha=0.84, vgg_weight=0.1):
...
def save_checkpoint(model, optimizer, epoch, loss, filename="checkpoint_2d_fusionv2.pth"):
    torch.save({
        'epoch': epoch,
...
def load_checkpoint(model, optimizer, filename="checkpoint_2d_fusionv2.pth"):
...
def train_epoch(model, train_loader, criterion, optimizer, scheduler, scaler, device, epoch, writer):
    ...
    for batch_idx, (inputs, targets, _) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch + 1}")):
        ...
        with torch.amp.autocast(device_type='cuda' if torch.cuda.is_available() else 'cpu'):
            ...
            loss, mse_losses, ssim_losses, vgg_losses, scale_weights = criterion(outputs, targets)
...
def main():
    ...
    criterion = AdaptiveMultiScaleLoss(num_scales=4).to(device)
    ...
    writer = SummaryWriter('runs/2d_unet_experiment_brats_fusionv2')
    ...
    torch.save(model.state_dict(), '2d_unet_model_brats_fusionv2.pth')

    with open('patient_normalization_params_2d_brats_fusionv2.json', 'w') as f:
