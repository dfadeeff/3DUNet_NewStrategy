...
def save_checkpoint(generator, discriminator, optimizer_G, optimizer_D, epoch, loss,
                    filename="checkpoint_cgan_enhancedEnh.pth"):
    torch.save({
        'epoch': epoch,
...
def load_checkpoint(generator, discriminator, optimizer_G, optimizer_D, filename="checkpoint_cgan_enhancedEnh.pth"):
...
def train_epoch(generator, discriminator, train_loader, criterion_G, criterion_D, optimizer_G, optimizer_D, scaler,
    ...
    for batch_idx, (inputs, targets, _) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch + 1}")):
        ...
        with autocast(device_type="cuda" if torch.cuda.is_available() else "cpu", enabled=torch.cuda.is_available()):
            ...
            # Label Smoothing
            valid_label = torch.full_like(real_validity, 0.9, device=device)
            fake_label = torch.full_like(fake_validity, 0.0, device=device)

            # Discriminator loss
            d_real_loss = criterion_D(real_validity, valid_label)
            d_fake_loss = criterion_D(fake_validity, fake_label)
        ...
        with autocast(device_type="cuda" if torch.cuda.is_available() else "cpu", enabled=torch.cuda.is_available()):
            ...
            valid_label = torch.full_like(fake_validity, 1.0, device=device)
            g_adv_loss = criterion_D(fake_validity, valid_label)
        ...
        # Calculate PSNR and SSIM for logging
        with torch.no_grad():
            mse_loss = nn.MSELoss()(fake_targets, targets)
            psnr = 10 * torch.log10(4 / mse_loss)
            ssim_val = ssim(fake_targets, targets, data_range=2.0, size_average=True)
...
def train(generator, discriminator, train_loader, val_loader, criterion_G, criterion_D, optimizer_G, optimizer_D,
          num_epochs, device, writer, config):
    scaler = GradScaler()
...
def main():
    ...
    writer = SummaryWriter('runs/cgan_training_Enh')
    ...
    torch.save(generator.state_dict(), 'generator_finalEnh.pth')
    torch.save(discriminator.state_dict(), 'discriminator_finalEnh.pth')

    # Save normalization parameters if needed
    with open('patient_normalization_paramsEnh.json', 'w') as f:
