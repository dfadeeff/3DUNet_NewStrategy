...
def save_checkpoint(generator, discriminator, optimizer_G, optimizer_D, scheduler_G, scheduler_D, epoch, loss,
                    filename="checkpoint_cgan_globalNormV5.pth"):
    torch.save({
        'epoch': epoch,
...
def load_checkpoint(generator, discriminator, optimizer_G, optimizer_D, scheduler_G, scheduler_D, filename="checkpoint_cgan_globalNormV5.pth"):
...
def train(generator, discriminator, train_loader, val_loader, criterion_G, criterion_D, optimizer_G, optimizer_D,
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            save_checkpoint(generator, discriminator, optimizer_G, optimizer_D, scheduler_G, scheduler_D, epoch, val_loss)
        else:
            patience_counter += 1
...
def main():
    ...
    writer = SummaryWriter('runs/cgan_globalNormV5')

    # Initialize SSIM module once with channel=1
    ssim_module = SSIM(data_range=2.0, channel=1).to(device)

    start_epoch = load_checkpoint(generator, discriminator, optimizer_G, optimizer_D, scheduler_G, scheduler_D)
    ...
    torch.save(generator.state_dict(), 'generator_globalNormV5.pth')
    torch.save(discriminator.state_dict(), 'discriminator_globalNormV5.pth')

    with open('patient_normalization_params_cgan_globalNormV5.json', 'w') as f:
