...
class UNet2D(nn.Module):
    def __init__(self, in_channels=3, out_channels=1, init_features=256, dropout_rate=0.1):
        super(UNet2D, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.vgg_loss = VGGLoss()
        self.scale_weights = nn.Parameter(torch.ones(8))  # Adjusted for 8 scales
        ...
        self.e8 = ConvBlock(init_features * 64, init_features * 128)

        # Decoder (upsampling with skip connections)

        self.d7 = ConvBlock(init_features * 128 + init_features * 64, init_features * 64)
        ...
        self.side_out8 = nn.Conv2d(init_features * 128, out_channels, kernel_size=1)
    ...
    def forward(self, x, target=None):
        ...
        e8 = self.e8(self.pool(e7))

        # Decoder with skip connections
        d7 = self.d7(torch.cat([F.interpolate(e8, size=e7.size()[2:], mode='bilinear', align_corners=True), e7], dim=1))
        ...
        side_out8 = F.interpolate(self.side_out8(e8), size=x.size()[2:], mode='bilinear', align_corners=True)
        ...
        out = self.final(
            torch.cat([side_out8, side_out7, side_out6, side_out5, side_out4, side_out3, side_out2, side_out1], dim=1))
        ...
        if self.training and target is not None:
            ...
            for i, (side_output, weight) in enumerate(zip(
                    [side_out8, side_out7, side_out6, side_out5, side_out4, side_out3, side_out2, side_out1, out],
                    weights)):
            ...
            return out, total_loss, [side_out8, side_out7, side_out6, side_out5, side_out4, side_out3, side_out2,
                                     side_out1]
        else:
            return out
