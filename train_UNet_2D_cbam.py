...
from model_UNet_2D_cbam import UNet2D, calculate_psnr, CombinedLoss
...
def save_checkpoint(model, optimizer, epoch, loss, filename="checkpoint_2d_cbam.pth"):
    torch.save({
        'epoch': epoch,
...
def load_checkpoint(model, optimizer, filename="checkpoint_2d_cbam.pth"):
...
def main():
    ...
    writer = SummaryWriter('runs/2d_unet_cbam')
    ...
    torch.save(model.state_dict(), '2d_unet_cbam.pth')

    with open('patient_normalization_params_2d_cbam.json', 'w') as f:
