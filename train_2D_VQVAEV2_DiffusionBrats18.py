...
from model_vqvaeV2 import ImprovedLatentDiffusion
...
def save_checkpoint(model, optimizer, epoch, loss, filename="checkpoint_improved_diffusion.pth"):
    torch.save({
        'epoch': epoch,
...
def load_checkpoint(model, optimizer, filename="checkpoint_improved_diffusion.pth"):
...
def train_epoch(model, train_loader, optimizer, scaler, device, epoch, writer, config):
    ...
    for batch_idx, (x_cond, targets, _) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch + 1}")):
        ...
        with autocast(enabled=True):
            ...
            recon_loss = F.l1_loss(output, targets) + (1 - ssim(output, targets, data_range=2.0))

            # Total loss
            loss = recon_loss + 0.1 * vq_loss + diffusion_loss
...
def main():
    ...
    writer = SummaryWriter('runs/improved_latent_diffusion')
    ...
    torch.save(model.state_dict(), 'improved_latent_diffusion.pth')
