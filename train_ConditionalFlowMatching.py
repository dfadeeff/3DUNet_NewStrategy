...
from model_ConditionalFlowMatching import AdvancedMRITranslationModel
...
class BrainMRI2DDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input_slice = np.stack([t1c, t2, flair], axis=0)
...
def save_checkpoint(model, optimizer, epoch, loss, filename="checkpoint_cond.pth"):
    torch.save({
        'epoch': epoch,
...
def load_checkpoint(model, optimizer, filename="checkpoint_cond.pth"):
...
def main():
    ...
    model = AdvancedMRITranslationModel(in_channels=3, out_channels=1, features=64).to(device)

    criterion = CombinedLoss(alpha=0.8).to(device)
    optimizer, scheduler = get_optimizer_and_scheduler(model, config, train_loader)

    writer = SummaryWriter('runs/advanced_mri_translation')
