...
def save_checkpoint(model, optimizer, epoch, loss, filename="checkpoint_multiscale_2d_corr.pth"):
    torch.save({
        'epoch': epoch,
...
def load_checkpoint(model, optimizer, filename="checkpoint_multiscale_2d_corr.pth"):
...
def main():
    ...
    train_dataset = BrainMRI2DDataset(train_root_dir, slice_range, patch_size, center_bias)
    val_dataset = BrainMRI2DDataset(val_root_dir, slice_range, patch_size, center_bias)
    ...
    writer = SummaryWriter('runs/2d_unet_experiment_full_image_multi_scale_corr_brats')
    ...
    with open('patient_normalization_params_2d_multi_scale_corr_brats.json', 'w') as f:
