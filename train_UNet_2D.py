...
from model_UNet_2D import UNet2D, calculate_psnr
...
def save_checkpoint(model, optimizer, epoch, loss, filename="checkpoint_2d.pth"):
    torch.save({
        'epoch': epoch,
...
def load_checkpoint(model, optimizer, filename="checkpoint_2d.pth"):
...
def main():
    ...
    num_epochs = 50
    learning_rate = 1e-4
    slice_range = (50, 130)  # Adjust this range based on your data
    patch_size = 128
    ...
    criterion = CombinedLoss()  # Using the CombinedLoss instead of nn.MSELoss()
    ...
    with open('patient_normalization_params_2d.json', 'w') as f:
