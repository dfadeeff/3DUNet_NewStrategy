...
class UNet2D(nn.Module):
    def __init__(self, in_channels=3, out_channels=1, init_features=32):
        ...
        self.final = nn.Conv2d(out_channels * 4, out_channels, kernel_size=1)
    ...
    def forward(self, x, target=None):
        ...
        if self.training and target is not None:
            # Adjust loss calculation as per previous section
            losses = []
            weights = [1 / (2 ** i) for i in range(5)]
            total_weight = sum(weights)
            for i, (side_output, weight) in enumerate(zip([side_out4, side_out3, side_out2, side_out1, out], weights)):
                mse_loss = F.mse_loss(side_output, target)
                content_loss, style_loss = self.vgg_loss(side_output, target)
                ssim_loss = 1 - ssim(side_output, target, data_range=1.0, size_average=True)
                total_loss = mse_loss + 0.1 * (content_loss + style_loss) + 0.1 * ssim_loss
                losses.append(total_loss * weight)
            total_loss = sum(losses) / total_weight
