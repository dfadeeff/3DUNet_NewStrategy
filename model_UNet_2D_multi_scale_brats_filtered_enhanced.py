...
class EnhancedCombinedLoss(nn.Module):
    def __init__(self, alpha=0.84, vgg_weight=0.1, psnr_weight=0.1, epsilon=1e-6):
    ...
    def forward(self, pred, target):
        ...
        psnr_loss = -calculate_psnr(pred, target)

        total_loss = self.alpha * mse_loss + (1 - self.alpha) * ssim_loss + \
                     self.vgg_weight * vgg_loss + self.psnr_weight * psnr_loss
...
def test_model():
    ...
    for i in range(input_tensor.shape[2]):
        ...
        with torch.no_grad():
            output_slice = model(slice_input)
