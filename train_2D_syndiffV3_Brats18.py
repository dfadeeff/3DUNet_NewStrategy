...
from model_2D_syndiffV3 import SynDiff2D
...
def save_checkpoint(model, optimizer, epoch, loss, filename="checkpoint_syndiff.pth"):
    torch.save({
        'epoch': epoch,
...
def load_checkpoint(model, optimizer, filename="checkpoint_syndiff.pth"):
...
def train(model, train_loader, val_loader, optimizer, num_epochs, device, writer, config):
    ...
    for epoch in range(num_epochs):
        train_loss, train_psnr, train_ssim = train_epoch(model, train_loader, optimizer, scaler, device, epoch, writer,
                                                         config)
...
def main():
    ...
    model = SynDiff2D(in_channels=3, out_channels=1, time_dim=256, n_steps=config['n_steps']).to(device)
    ...
    torch.save(model.state_dict(), 'syndiff_model.pth')

    with open('patient_normalization_params_syndiff.json', 'w') as f:
        json.dump(train_dataset.normalization_params, f)

    torch.save(model.state_dict(), 'syndiff_model.pth')

    with open('patient_normalization_params_cgan_globalNormV3.json', 'w') as f:
        json.dump(train_dataset.normalization_params, f)
