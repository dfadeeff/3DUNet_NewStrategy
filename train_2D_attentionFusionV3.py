...
from model_2D_attentionFusionV3 import Generator, Discriminator, PerceptualLoss
...
def save_checkpoint(generator, discriminator, optimizer_G, optimizer_D, epoch, loss,
                    filename="checkpoint_attentionFusionV3.pth"):
    torch.save({
        'epoch': epoch,
...
def load_checkpoint(generator, discriminator, optimizer_G, optimizer_D, filename="checkpoint_attentionFusionV3.pth"):
...
def main():
    ...
    writer = SummaryWriter('runs/cgan_attFV3')
    ...
    torch.save(generator.state_dict(), 'generator_attFV3.pth')
    torch.save(discriminator.state_dict(), 'discriminator_attFV3.pth')

    with open('patient_normalization_params_attFV2.json', 'w') as f:
