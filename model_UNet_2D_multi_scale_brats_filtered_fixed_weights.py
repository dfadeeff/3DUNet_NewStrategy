...
class UNet2D(nn.Module):
    def __init__(self, in_channels=3, out_channels=1, init_features=32):
        ...
        self.scale_weights = torch.tensor([1.0, 0.8, 0.6, 0.4, 0.2, 0.1, 0.05])  # Fixed weights
    ...
    def forward(self, x, target=None):
        ...
        if self.training and target is not None:
            losses = []
            weights = self.scale_weights.to(x.device)
            epsilon = 1e-8
            for i, (side_output, weight) in enumerate(zip(
                    [side_out7, side_out6, side_out5, side_out4, side_out3, side_out2, side_out1], weights)):
            ...
            # Handle the final output separately
            final_output = out.clamp(0 + epsilon, 1 - epsilon)
            final_mse_loss = F.mse_loss(final_output, target_clamped)
            final_ssim_loss = 1 - ssim(final_output, target_clamped, data_range=1.0, size_average=True)
            content_loss, style_loss = self.vgg_loss(final_output, target_clamped)
            final_loss = (final_mse_loss + 0.1 * final_ssim_loss) + 0.1 * (content_loss + style_loss)
