...
from model_2D_syndiffV8 import SynDiff2D
...
def save_checkpoint(model, optimizer, scheduler, epoch, loss, filename="checkpoint_syndiffV8.pth"):
    torch.save({
        'epoch': epoch,
...
def load_checkpoint(model, optimizer, filename="checkpoint_syndiffV8.pth"):
...
def main():
    ...
    writer = SummaryWriter('runs/syndiffV8')
    ...
    torch.save(model.state_dict(), 'syndiff_modelV8.pth')

    with open('patient_normalization_params_syndiffV8.json', 'w') as f:
