...
from model_2D_cGAN import GeneratorUNet, Discriminator, PerceptualLoss  # Import from your model.py
...
class BrainMRI2DDataset(Dataset):
    ...
    def normalize_slice(self, tensor, epsilon=1e-7):
        ...
        normalized_tensor = 2 * (tensor - min_val) / (max_val - min_val + epsilon) - 1  # Normalize to [-1, 1]
...
def save_checkpoint(generator, discriminator, optimizer_G, optimizer_D, epoch, loss, filename="checkpoint_cgan.pth"):
    torch.save({
        'epoch': epoch,
...
def load_checkpoint(generator, discriminator, optimizer_G, optimizer_D, filename="checkpoint_cgan.pth"):
...
def main():
    ...
    writer = SummaryWriter('runs/cgan_training')
    ...
    with open('patient_normalization_params_cgan.json', 'w') as f:
