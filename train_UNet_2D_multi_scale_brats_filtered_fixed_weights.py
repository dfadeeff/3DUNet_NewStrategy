...
from model_UNet_2D_multi_scale_brats_filtered_fixed_weights import UNet2D, calculate_psnr, CombinedLoss
...
def save_checkpoint(model, optimizer, epoch, loss, filename="checkpoint_2d_multi_scale_brats_filtered_fixed_weights.pth"):
    torch.save({
        'epoch': epoch,
...
def load_checkpoint(model, optimizer, filename="checkpoint_2d_multi_scale_brats_filtered_fixed_weights.pth"):
...
def main():
    ...
    writer = SummaryWriter('runs/2d_unet_experiment_brats_filtered_fixed_weights')
    ...
    torch.save(model.state_dict(), '2d_unet_model_multi_scale_brats_filtered_fixed_weights.pth')

    with open('patient_normalization_params_2d_multi_scale_brats_filtered_fixed_weights.json', 'w') as f:
