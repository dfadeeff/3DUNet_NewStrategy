...
from model_2D_syndiffV4 import SynDiff2D
...
def train_epoch(model, train_loader, optimizer, scaler, device, epoch, writer, config):
    ...
    for batch_idx, (inputs, targets, _) in enumerate(tqdm(train_loader)):
        ...
        with autocast(device_type="cuda" if torch.cuda.is_available() else "cpu", enabled=torch.cuda.is_available()):
            # 1. Generate noise for target T2 image
            noise = torch.randn_like(targets)  # Shape: [B, 1, H, W]
    ...
    return (running_loss / len(train_loader),
            running_psnr / len(train_loader),
            running_ssim / len(train_loader))
