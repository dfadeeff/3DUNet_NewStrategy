...
from model_2D_attentionFusion import Generator, Discriminator, PerceptualLoss
...
def save_checkpoint(generator, discriminator, optimizer_G, optimizer_D, epoch, loss,
                    filename="checkpoint_attentionFusion.pth"):
    torch.save({
        'epoch': epoch,
...
def load_checkpoint(generator, discriminator, optimizer_G, optimizer_D, filename="checkpoint_attentionFusion.pth"):
...
def train_epoch(generator, discriminator, train_loader, criterion_G, criterion_D, optimizer_G, optimizer_D, scaler,
    ...
    for batch_idx, (inputs, targets, indices) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch + 1}")):
        ...
        with autocast(device_type="cuda" if torch.cuda.is_available() else "cpu", enabled=torch.cuda.is_available()):
            ...
            # Perceptual loss
            g_perc_loss = criterion_G(fake_targets, targets)
...
def validate(generator, val_loader, criterion_G, device, epoch, writer, config, ssim_module):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets, indices) in enumerate(val_loader):
            ...
            # Perceptual loss
            perceptual_loss = criterion_G(fake_targets, targets)
...
def main():
    ...
    # Initialize models
    generator = Generator(in_channels=3, out_channels=1, features=128).to(device)
    ...
    writer = SummaryWriter('runs/cgan_attF')
    ...
    torch.save(generator.state_dict(), 'generator_attF.pth')
    torch.save(discriminator.state_dict(), 'discriminator_attF.pth')

    with open('patient_normalization_params_attF.json', 'w') as f:
