...
class PatchBrainMRIDataset(Dataset):
        ...
        for subject_folder in sorted(os.listdir(self.root_dir)):
            ...
            if os.path.isdir(subject_path):
                ...
                for filename in os.listdir(subject_path):
                    # Skip hidden files and AppleDouble files
    ...
    def create_patches_info(self):
        ...
        for idx, data_entry in enumerate(self.data_list):
            print(f"Processing data entry {idx}: {data_entry['FLAIR']}")
            try:
                ...
                print(f"Error processing file {data_entry['FLAIR']}: {str(e)}")
        return patches_info
...
def visualize_batch(inputs, targets, outputs, epoch, batch_idx, writer):
    ...
    # Plot FLAIR
    axes[0].imshow(input_slices[0], cmap='gray')
    axes[0].set_title('FLAIR')
    axes[0].axis('off')

    # Plot T1c
    axes[1].imshow(input_slices[1], cmap='gray')
    axes[1].set_title('T1c')
    axes[1].axis('off')

    # Plot T2
    axes[2].imshow(input_slices[2], cmap='gray')
    axes[2].set_title('T2')
    axes[2].axis('off')

    # Plot ground truth T1
    axes[3].imshow(target_slice, cmap='gray')
    axes[3].set_title('Ground Truth T1')
    axes[3].axis('off')

    # Plot generated T1
    axes[4].imshow(output_slice, cmap='gray')
    axes[4].set_title('Generated T1')
    axes[4].axis('off')
    ...
    writer.add_image(f'Visualization/Epoch_{epoch}_Batch_{batch_idx}', image.transpose(2, 0, 1), epoch)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch + 1}/{num_epochs}")):
            ...
            if batch_idx % 10 == 0:
                ...
                visualize_batch(inputs[:, :, mid_slice],
                                targets[:, :, mid_slice],
                                outputs[:, :, mid_slice],
                                epoch, batch_idx, writer)
...
def main():
    ...
    writer = SummaryWriter('runs/multi_scale_unet_optimized_batch16')

    start_epoch = load_checkpoint(model, optimizer)

    train(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs - start_epoch, device, writer)

    torch.save(model.state_dict(), 'multi_scale_unet3d_model_batch16.pth')
