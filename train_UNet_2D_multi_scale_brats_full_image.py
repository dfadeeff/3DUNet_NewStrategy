...
class BrainMRI2DDataset(Dataset):
    def __init__(self, root_dir, slice_range=(2, 150)):
    ...
    def __getitem__(self, idx):
        ...
        input_slice, min_val, max_val = self.normalize_with_percentile(input_slice)
        if input_slice is None:
            return self.__getitem__((idx + 1) % len(self))
        target_slice, target_min, target_max = self.normalize_with_percentile(target_slice)
...
def save_checkpoint(model, optimizer, epoch, loss, filename="checkpoint_multiscale_2d_brats_full_image.pth"):
    torch.save({
        'epoch': epoch,
...
def load_checkpoint(model, optimizer, filename="checkpoint_multiscale_2d_brats_full_image.pth"):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (inputs, targets, indices) in enumerate(
            ...
            with autocast(device_type='cuda'):
                # Adjusted to expect only two outputs (output and loss)
...
def main():
    ...
    config = {
        'batch_size': 16,  # Adjusted batch size for full images
        'num_epochs': 200,
        'learning_rate': 1e-4,
        'slice_range': (2, 150),
        'weight_decay': 1e-5,
    ...
    writer = SummaryWriter('runs/2d_unet_experiment_brats_full_image')

    start_epoch = load_checkpoint(model, optimizer)

    train(model, train_loader, val_loader, criterion, optimizer, scheduler, config['num_epochs'] - start_epoch,
          device,
          writer, train_dataset, val_dataset)

    torch.save(model.state_dict(), '2d_unet_model_multi_scale_brats_full_image.pth')

    with open('patient_normalization_params_2d_multi_scale_brats_full_image.json', 'w') as f:
