...
class EnhancedResBlock(nn.Module):
    def __init__(self, in_channels, out_channels, use_attention=False, drop_path=0.1):
        ...
        # Fixed Normalization - Initialize with correct channel dimensions
        self.norm1 = nn.GroupNorm(8, in_channels)  # Changed from out_channels to in_channels
        ...
        self.attention = CrossAttention(out_channels) if use_attention else None

    def forward(self, x):
        identity = self.downsample(x)

        # First conv block with proper order
        out = self.norm1(x)  # Normalize input channels
        out = self.act(out)
        out = self.conv1(out)  # Changes channel dimension
        out = self.dropout(out)

        # Second conv block
        out = self.norm2(out)  # Now normalize out_channels
        out = self.act(out)
        out = self.conv2(out)
        out = self.dropout(out)
...
class SynDiff2D(nn.Module):
    ...
    @torch.no_grad()
    def ddim_sampling(self, x, num_inference_steps=50, eta=0.0):
        """
        ...
        sample = torch.randn_like(x[:, :1]).clamp(-1, 1)  # Clamp for stability

        for i, t in enumerate(timesteps):
            # Get next timestep
            t_next = timesteps[i + 1] if i < len(timesteps) - 1 else torch.tensor(0, device=x.device)

            # Current alpha values
            alpha_t = self.alpha_schedule(t)
            alpha_next = self.alpha_schedule(t_next)

            # Predict noise
            x_input = torch.cat([x, sample], dim=1)
            pred_noise = self.forward(x_input, t.float().unsqueeze(0))

            # Predict x0 (clean image)
            x0_pred = (sample - torch.sqrt(1 - alpha_t) * pred_noise) / torch.sqrt(alpha_t)
            x0_pred = x0_pred.clamp(-1, 1)  # Clamp for stability

            # DDIM formula
            sigma_t = eta * torch.sqrt((1 - alpha_next) / (1 - alpha_t) * (1 - alpha_t / alpha_next))
            noise = torch.randn_like(sample) if eta > 0 else 0  # Controlled noise

            # Compute xt_next deterministically
            c1 = torch.sqrt(alpha_next)
            c2 = torch.sqrt(1 - alpha_next - sigma_t.pow(2))
            sample = c1 * x0_pred + c2 * pred_noise + sigma_t * noise
    ...
    def fast_sampling(self, x, num_inference_steps=50):
        return self.ddim_sampling(x, num_inference_steps, eta=0.0)  # Fully deterministic
